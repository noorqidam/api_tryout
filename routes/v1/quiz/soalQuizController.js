...
router.post('/',[
  ...
  if (!errors.isEmpty()) {
    ...
    try {
      ...
      const cek_soal = req.body.soal.filter(item => {
        return item.type_soal === 'PG' ? item.jawabans.length < 1 ? true : false : true
      })
      if (cek_soal.length > 0) {
        return res.status(422).json({
          error: 'Unprocessable Entity',
          message: 'Terdapat soal yang bukan soal PG atau opsi jawaban tidak ada'
        })
      } else {
