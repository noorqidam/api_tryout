						...
						GetPesertaQuiz.forEach((item, index) => {
							...
							if (checkJawaban.length) {
								...
								data.push({
									...
									name: checkJawaban[0].name || 'No Data',
									nama_sekolah: checkJawaban[0].school,
									kelas: checkJawaban[0].kelas,
									sub_kelas: checkJawaban[0].sub_kelas,
									totalSoal: totalSoal,
									answered: checkJawaban[0].count,
									score: (checkJawaban[0].total_point / totalSoal * 100).toFixed(2)
								})
							} else {
								tidakLulus += 1
								data.push({
							...
							if (index == GetPesertaQuiz.length - 1) {
