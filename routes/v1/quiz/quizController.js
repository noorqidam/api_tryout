...
router.get('/', async(req, res) => {
    ...
    try {
        const CountQuiz = await Quiz.countDocuments({ deleted : false})
        const DataUjian = await Quiz.find({ deleted: false })
            .limit(limit)
            .skip(offset)
            .sort({ id: -1 });
...
router.get('/:id', async(req, res) => {
    ...
    try {
        const GetQuiz = await Quiz.findOne({ _id: id, deleted: false })
...
router.post('/',
    ...
    UploadImage.sendUploadToGCS, [
        check('judul_quiz').not().isEmpty().withMessage('require'),
        check('quiz_slug').not().isEmpty().withMessage('require'),
        check('category_quiz').not().isEmpty().withMessage('require'),
...
router.put('/:id', [
    ...
    if (!errors.isEmpty()) {
        ...
        try {
            ...
            let is_premium = req.body.is_premium
            const GetQuiz = await Quiz.findById({ _id: id, deleted: false })

            const UpdateQuiz = {
                judul_quiz: req.body.judul_quiz,
                quiz_slug: req.body.quiz_slug,
                quiz_banner: req.body.quiz_banner,
                type_quiz: req.body.type_quiz,
                is_premium: req.body.is_premium ? req.body.is_premium : false,
                is_private: req.body.is_private ? req.body.is_private : false,
                category_quiz: req.body.category_quiz,
            ...
            if (GetQuiz) {
                if (is_private) {
                    ...
                    console.log(findAccessQuiz)
                    if (findAccessQuiz) {
                        findAccessQuiz.updateOne({
                            acces_pin: req.body.pin
                        })
                    } else {
                        const CreateAccesQuiz = new QuizAccess({
