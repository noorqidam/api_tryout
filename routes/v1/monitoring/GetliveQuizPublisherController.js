...
router.get('/', async (req, res) => {
  ...
  if (auth.role ==='ADMIN' || auth.role =='STAF') {
    try {
      const GetLiveTest = await Quiz.find({ publish: true ,penyelenggara: auth.accountkey, start_date: { $gte:now , $lt : maxNow}})
        .limit(limit)
        .skip(offset)
        .sort({ id : -1})
    ...
    try {
      const GetLiveTest = await Quiz.find({  publish: true, penyelenggara: auth.accountkey, createdBy: auth.username,start_date: { $gte:now , $lt : maxNow}})
        .limit(limit)
        .skip(offset)
        .sort({ id : -1})
